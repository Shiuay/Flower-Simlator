...
argent = 5
date   = [1, 8, 2016, "Lundi", 52, 0, 0]  # [Jour, Mois, Années, Jour, rectangle afficheur calendrier, ticks, jour]
Meteo  = ["Dégagé", 1]
    ...
    while continuer:
        ...
        if date[5] == int(parametres["VitesseJour"]):
            ...
            if degage(Meteo):
                ...
                label_meteo.config(image=photo_nuage)
            elif pluvieux(Meteo):
...
photo_nuage  = PhotoImage(file="image\\nuage.png")
...
boutons_fleur = list()
for i in range(12):
    boutons_fleur.append(Radiobutton(canvas, image=photo_pot, bg='#F0F0F0', variable=choix_fleur, value=i))
...
label_gold = Label(fenetre, anchor=NW, width=40, height=5, text="${}".format(argent), relief=FLAT, fg='black', command=None, font=("Helvetica", 16))
...
LMois  = (("Janvier", 31),("Février", 28),("Mars", 31),("Avril", 30),("Mai", 31),("Juin", 30),("Juillet", 31),("Août", 31),("Septembre", 30),("Octobre", 31),("Novembre", 30),("Décembre", 31))
...
for i in range(6):
    for u in range(7):
        Calendrier.create_text(x,y, tags=a) # premier tag a partir de 10. me demande pas. python powa
        a += 1
        x += 35
    y += 30
    x = 20
...
for i in range(6):
    for u in range(7):
        Calendrier.create_rectangle(x0,y0,x1,y1, tags=a, fill='', outline='red', width=0)
        x0 += 35
        x1 += 35
        a  += 1
    y0 += 30
